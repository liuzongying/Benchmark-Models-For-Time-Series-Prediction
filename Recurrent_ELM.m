function [Final_result,average_test_SMAPE]...
    = Recurrent_ELM(Data_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction)

%[Final_result,average_test_SMAPE] = Recurrent_ELM('nonPJM.csv', 0, 20, 'sig')
...
d =load(Data_File);
data = d(1:end,:); 
predictsize =20;         %the number of training X (cloumn)
period = 5;              %the number of predict size
...
end_training_time(m+1,1) = cputime;      %   Calculate CPU time (seconds) spent for training ELM
...
end_testing_time(m+1,1) = cputime;          %   Calculate CPU time (seconds) spent by ELM predicting the whole testing data
