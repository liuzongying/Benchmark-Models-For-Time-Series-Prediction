...
irange_l=0.6e-6; % Lower bound of initialization scope
irange_r=100;  % Upper bound of initialization scope
xmax=100;   % Upper bound of the search scope
xmin=0.6e-6;  % Lower bound of the search scope
...
%%%%%%%%%%%%%%% Initialization of the particle swarm %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %  gbest=zeros(1,dimension); % Initialize the gbest poistion vector
    ...
    for i=1:popsize
        f_x(i)=Recurrent_KELM_with_reservoir_computing('norpalm_oil_7days.csv', 0, 'RBF_kernel',x(i,1),x(i,2));   %Calculate the fitness value of the current position of the particle
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
        ...
        elseif x(i,2)>=100;
            ...
            f_x(i)=Recurrent_KELM_with_reservoir_computing('norpalm_oil_7days.csv', 0, 'RBF_kernel',x(i,1),x(i,2));  % Calculate the fitness value of the particle's current position
            if (f_x(i)<f_pbest(i)) 
    ...
    data(run,t)=f_gbest;  % Record the fitness value of the gbest at each iteration at this run
     % Display Iteration Information
     disp(['Iteration ' num2str(t) ': Best Cost = ' num2str(f_gbest) ', Best position =' num2str(gbest)]);
      BestCost.Iteration(run,t) = f_gbest;
%     f_gbest
    end
    % Display Number of run information
    Best_cost(run,1) = f_gbest;
    disp(['Norun ' num2str(run) ': Best Cost = ' num2str(f_gbest) ', Best position =' num2str(gbest)]);
%      f_gbest
    result.f_gbest(run,1) = f_gbest;
    result.gbest(run,:) = gbest;
end
num_out = find(result.f_gbest==min(result.f_gbest));  
BEST_COST = result.f_gbest(num_out,1);
BEST_POSITION = result.gbest(num_out,:);
